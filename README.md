...
I fondamenti di un motore pneumatico e i suoi limiti tradizionali.

La soluzione innovativa implementata in questo prototipo.

I dettagli costruttivi e i risultati sperimentali.

La roadmap futura verso un sistema ibrido.
...
Immissione: L'aria ad alta pressione viene introdotta nella camera di espansione (cilindro).

Espansione: L'aria si espande, spingendo il pistone.

Scarico: L'aria esausta (a pressione più bassa) viene evacuata dal cilindro.

Trasformazione: Il moto rettilineo del pistone viene convertito in moto rotatorio da un meccanismo biella-manovella.
...
Sensori che rilevano in tempo reale la posizione del pistone.

Un cervello elettronico (Arduino) che decide, in base a questa lettura e a logiche programmabili, il momento esatto per agire.

Attuatori (elettrovalvole) che aprono e chiudono i passaggi dell'aria su comando digitale.
...
Fase 1 (Immissione): Una piccola quantità di combustibile liquido (es. etanolo) viene iniettata e miscelata con l'aria in ingresso.

Fase 2 (Accensione): La miscela viene accesa (con candeletta o scintilla) all'interno del cilindro.

Fase 3 (Espansione Ibrida): La combustione riscalda violentemente l'aria, causando un ulteriore, drammatico aumento di pressione e volume. È questa espansione termica aggiuntiva che fornisce la maggior parte del lavoro, moltiplicando potenza e autonomia.
